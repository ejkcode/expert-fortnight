...
describe('13: /api', () => {
   ...
   test('GET: 200 - each key representing an endpoint has a description, queries and exampleResponse propety, excluding the `GET /api` endpoint which only has a description property, and the POST and DELETE endpoints which have an additional "exampleBody" property', () => {
        return request(app)
            .get('/api')
            .expect(200)
            .then(({body}) => {
                for (const endpoint in body.endpoints) {
                    if (endpoint != "GET /api") {
                        if (endpoint === "POST /api/reviews/:review_id/comments" || endpoint === "DELETE /api/comments/:comment_id") {
   ...
   test('GET: 200 - the exampleBody property, where applicable, should be an object with a single key', () => {
    return request(app)
            .get('/api')
            .expect(200)
            .then(({body}) => {
                for (const endpoint in body.endpoints) {
                    if (endpoint === "POST /api/reviews/:review_id/comments" || endpoint === "DELETE /api/comments/:comment_id") {
                        const exampleResponsePropertyKeys = Object.keys(body.endpoints[endpoint].exampleBody)
                        expect(exampleResponsePropertyKeys.length).toBe(1);
                    };
                };
            });
